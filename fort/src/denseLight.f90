module denseLight
    ...
    subroutine selectElements(self, elementlist)
        ...
        real(WP)                       :: new_y_olds(size(elementlist),self%n_segments), &
                                          ...
                                          newFrevs(0:self%fs(1)-1, &
                                                   0:size(elementlist)-1, &
                                                   self%fs(3))
        ...
        self%fs = [ncoeff, self%ndim, self%n_segments]
    ...
    subroutine convert_from_object(self, ODESol)
        ...
        do i=1, self%n_segments
            self%Frevs(:,:,i) = real(ODESol%interpolants(i)%Frev, wp)
    ...
    function call_(self, t) result(res)
        ...
        !$OMP PARALLEL DO
        do iter=1,size(t)
            res(:,iter) = self%scall(t(iter),segments(iter))
        end do
        !$OMP END PARALLEL DO
    end function call_
    function scall(self, t,segment) result(res)
        class(lightSol), intent(inout) :: self
        real(WP),           intent(in) :: t
        integer,            intent(in) :: segment
        real(WP) :: resarray(self%ndim,1), res(self%ndim)
        resarray = self%call_impl([t],segment)
        res = resarray(:,1)
    end function scall
    function call_impl(self, t,segment) result(res)
        ...
        y = 0._WP
        do i=0,size(self%Frevs,1)-1
            forall (j=1:size(t)) y(:,j) = y(:,j) + self%Frevs(i,:,segment)
            if (mod(i,2)== 0) then
